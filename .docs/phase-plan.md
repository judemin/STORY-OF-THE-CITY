...
## Phase 1 — 구조 설계
- [ ] 모듈 분리: constants / state / world / render / ui / time
- [ ] ES module import/export 구조 확정
- [ ] 순환 import 없음
- [ ] 빌드 없이 정적 서버에서 실행 가능

## Phase 2 — 기본 UI & 타이머
- [ ] HUD: Start/Pause 버튼, 시간 표시
- [ ] `state.running` 토글 정확히 동작
- [ ] `state.totalSecs` 누적 정확히 동작
- [ ] HUD 높이 Canvas 레이아웃에 반영

## Phase 3 — 코어 상태/시간 흐름
- [ ] Stage 0–4 전환 (0/2/4/6/8h 기준)
- [ ] `getTimeOfDay()` 실제 시각 기반
- [ ] dawn/morning/day/afternoon/evening/night 분기
- [ ] stage + tod → render에 전달

## Phase 4 — 월드/맵 생성
- [ ] 시드 매 호출마다 새로 생성
- [ ] 42×42 그리드, 타일 타입 할당
- [ ] Hero block 고정 배치 (lot 타일에만)
...
- [ ] 건물 face 색상 (hexRgb 직접 계산, rgba 없음)
- [ ] Backdrop 대기 틴트 렌더링
